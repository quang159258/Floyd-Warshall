...
void n_Box_NoBorder(int x, int y, int w, int h, int b_color, vector<vector<int>>Matrix_Weight)
{
    Box(x, y, w, h, b_color, "");
    int n = Matrix_Weight.size();
    int l = MaxLengthMatrix(Matrix_Weight) + 1;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            gotoXY(x + 1+l*j, y + 1 + i);
            cout <<setw(l)<<right<< Matrix_Weight[i][j];
        }
    }
}
